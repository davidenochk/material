   ...
   * });
    ...
    angular.module('material.components.datepicker').config(function($provide) {
        ...
        DateLocaleProvider.prototype.$get = function($locale, $filter) {
             ...
             * @returns {string}
             */
            function defaultFormatDate(date, timezone, isMonth) {
                if (!date) {
                    return '';
                }
                ...
                // If month selector, display selected month as MMM, yyyy format
                if(isMonth){
                    return $filter('date')(formatDate, 'MMM, yyyy', timezone);
                }
